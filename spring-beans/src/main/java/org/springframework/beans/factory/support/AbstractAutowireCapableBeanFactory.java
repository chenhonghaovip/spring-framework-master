...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	protected Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		try {
			// 这里又是一个核心逻辑：doCreateBean 创建Bean，创建完成后 直接短路掉返回
	...
	protected Object doCreateBean(final String beanName, final RootBeanDefinition mbd, final @Nullable Object[] args)
		...
		//bean包装类，用BeanWrapper来持有创建出来的Bean对象
		...
		try {
			...
			// 关于populateBean和initializeBean的详解，初始化bean
	...
	protected BeanWrapper createBeanInstance(String beanName, RootBeanDefinition mbd, @Nullable Object[] args) {
		...
		// 如果工厂方法不为空，则使用工厂方法初始化策略并返回创建结果
		// 当通过@Bean注解方式生成bean时，factoryMethodName为bean创建方法
	...
	protected void populateBean(String beanName, RootBeanDefinition mbd, @Nullable BeanWrapper bw) {
		...
		if (!mbd.isSynthetic() && hasInstantiationAwareBeanPostProcessors()) {
			// 遍历所有的BeanPostProcessor实现类，获取InstantiationAwareBeanPostProcessor类型的实现类
		...
		if (hasInstAwareBpps) {
			...
			for (BeanPostProcessor bp : getBeanPostProcessors()) {
				if (bp instanceof InstantiationAwareBeanPostProcessor) {
					InstantiationAwareBeanPostProcessor ibp = (InstantiationAwareBeanPostProcessor) bp;
					// 执行后置处理器的postProcessProperties方法
	...
	protected Object initializeBean(final String beanName, final Object bean, @Nullable RootBeanDefinition mbd) {
		if (System.getSecurityManager() != null) {
			...
			// 判断bean是否实现aware接口，若实现，进行方法回调，赋值
			invokeAwareMethods(beanName, bean);
		}
		...
		try {
			// 进行初始化操作，eg：init-method方法或者@postConstruct方法
	...
	protected void invokeInitMethods(String beanName, final Object bean, @Nullable RootBeanDefinition mbd)
		...
		// 如果bean实现了InitializingBean接口，且包含afterPropertiesSet，则执行afterPropertiesSet（）方法进行初始化操作
